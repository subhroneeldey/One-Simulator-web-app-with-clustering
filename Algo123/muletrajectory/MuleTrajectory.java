...
public class MuleTrajectory {
	...
	*/
	...
	Cserv contains information about the number of new IDB's visited by the iTH mule . Note: Not needed according to the
	...
	*/
			...
			*/
			...
			while (Found == true) {
				...
				//Add a special case when the only IDB in the trajectory is the group centre
					...
					if (i > 1 && this.trajectory[i].equals(this.trajectory[i - 1])) 
					{
						Flag = false;
					}
	...
	private boolean FindNearestNeighbourVisitedInCurrentTrail() 
	{
		...
		for (int x : X) 
		{
			...
			// check for feasibility of equation
			this.Tserv[this.muleCount] = Math.abs((tempTravel * this.sys.g * this.sys.p* this.sys.n / 60)/((this.sys.rswifi - this.sys.g * this.sys.p * this.sys.n/60)));
	...
	private boolean FindNearestNeighbourVisitedInPreviousTrail() 
	{
		...
		{
			float weight = Graph.aMat[MuleTrajectory.cIDB][i];
			if (weight > 0 && MuleTrajectory.visitP.contains(i) && !this.visitC.contains(i) && !MuleTrajectory.Explored.contains(i))
				X.add(i);
		}
