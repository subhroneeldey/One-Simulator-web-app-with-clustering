...
public class SimMap implements Serializable {
	...
	public MapNode getClosestMapNode(Coord coord) {
		
		MapNode minMapNode = this.nodes.get(0);
		double mindist = minMapNode.getLocation().distance(coord);
		int size=this.nodes.size();
		
		for (int i = 1; i < size; i++) {
			MapNode mapNode = this.nodes.get(i);
			double dist = mapNode.getLocation().distance(coord);
			if (dist < mindist) {
				mindist = dist;
				minMapNode = mapNode;
			}
