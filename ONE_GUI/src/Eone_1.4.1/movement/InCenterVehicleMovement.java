...
public class InCenterVehicleMovement extends MapBasedMovement {
	...
	private ArrayList<Coord[]> wayArray;
	...
	void parseWaypoints(String[] values) {
		// String[] values = settings.getCsvSetting("waypoints");
		this.waypoints = new Coord[values.length / 2];
		for (int i = 0; i < values.length / 2; i++) {
			this.waypoints[i] = new Coord(Double.parseDouble(values[i * 2]),
	...
	public double nextPathAvailable() {
		...
		if (waitTime == null) {
			...
			// System.out.println(waitTime[waypointIndex]);
			return SimClock.getTime() + waitTime[waypointIndex];
		}
	}
	...
	public void setHost(DTNHost host) {
		...
		int index = address - firstCD;

		String[] values = settings.getCsvSetting("waypoints"
				+ Integer.toString(index+1));
		parseWaypoints(values);
		
		/*
		 * for (int i = 0; i < totDM; i++) { String[] values =
		 * settings.getCsvSetting("waypoints" + Integer.toString(i + 1)); //
		 * reading the non GCs visited // by each data mule // for(int
		 * j=0;j<values.length;j++) // System.out.println(values[j]);
		 * parseWaypoints(values); }
		 */

		String waitTimeSettings = "waitTime" + (index + 1);
		if (this.settings.contains(waitTimeSettings)) {
			String[] wt = this.settings.getCsvSetting(waitTimeSettings);
			waitTime = new int[wt.length];
			for (int j = 0; j < wt.length; j++) {
				waitTime[j] = Integer.parseInt(wt[j]);
				// System.out.print(waitTime[j]+" ");
			}
		}
	...
	protected InCenterVehicleMovement(InCenterVehicleMovement rem) {
		...
		this.waitTime = rem.waitTime;
		this.waypoints = rem.waypoints;
		// this.wayArray = rem.wayArray;

		this.totDM = rem.totDM;
		this.firstCD = rem.firstCD;
		this.address = rem.address;
	...
	private MapNode getHome() {
		/*
		 * if (PDMConfig.getCenters().get(homeCenterType) == null) // return
		 * null;
		 * 
		 * List<MapNode> centers = PDMConfig.getCenters().get(homeCenterType); h
		 ...
		 * targetCenter = trgs.get(rng.nextInt(trgs.size())); targetCenter =
		 * trgs.get(nextTarget++); }
		 * 
