...
function makeGrid(sw, ne) {
    ...
    //stores rectangle of each grid cell
    regions = [];
    gridSnappedPoints = [];

    var i;
    // row point ( latitude) for creating grid points
    for (i = minlat; i <= maxlat; i += point_interval) {
        ...
        var j;
        // col point ( longitude) for creating grid points
        for (j = minlon; j <= maxlon; j += point_interval) {
            ...
            //placing blue marker at each grid point
            placeColorMarker(latlng, "blue");
        }
        // inserting the new grid points row
        grid.push(gridrow);
        gridSnappedPoints.push(gridSnappedRow);
        // last column if it wasnt included in the loop
        if (j != maxlon) {
        ...
        //inserting the new grid cells rectangle row
        regions.push(regionrow);
    }
    // last row if it wasnt included in the loop
    if (i != maxlat) {
        var regionrow = [];
    ...
    // function to snap grid points to check if they are on road
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (!jQuery.isEmptyObject(data)) {
                ...
                // place orange color marker on snapped latlng
...
function floodFilling() {
    ...
    //array of edges to request for direction
    edges = [];
    //connect edges in each row
    for (var r = 0; r < gridrows; r++) {
        var source = null,
            c = 0;
        // find a source among snapped points
        do {
        ...
        // connect with next point and take it as new source
        for (; c < gridcols; c++) {
...
function reqEdgesDirection(edgeindex) {
    ...
    // last request time out is set
    setLastRequestTime();
    // edge directions request properties options
    var request = {
    ...
    // async call to google maps server for direciton of route
    directionsService.route(request, function (response, status) {
        // if user wants to stop extracting then stop recursion and  start image request
        if (stopDirections) {
            startMapImageRequestFull();
            return;
        }
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            reqCount++;
            console.log(response.routes.length);
            //process each response in a recursive function
            var routesLength = response.routes.length;
...
function processEdgeResponse(index, response, edgeindex) {
    if (index >= response.routes.length) {
        //if all the routes of the direction response is processed request for next edge
        setTimeout(function () {
            reqEdgesDirection(edgeindex + 1);
        }, getTimeout());
        return;
    ...
    var kmlpoints = [],
        latlngs = [];
    //render the specific route being processed
    var directionsDisplay = new google.maps.DirectionsRenderer({
        map: map,
        directions: response,
        preserveViewport: true,
    ...
    for (var j = 0, n = dirroute.length; j < n; j++) {
        var latlng = dirroute[j];
        //checks if latlng within previous routes
        overlapped = checkAllboxContains(latlng);

        //if overlapped, dont include the latlng in to-be-written path array
        if (overlapped) {
            overlappedlatlngs.push(latlng);
            ...
            //to extend bounds if latlng is out of current bounds
            calcMaxBoundaries(latlng);
    ...
    // if we get any unoverlapped route
    // add it to map.kml file
    if (kmlpoints.length > 0) {
        var boxes = routeboxer.box(dirroute, boxsize);
        //add the route boxes of current route to allboxes variable
        addToAllBoxes(boxes);
        //if (!overlapped) {
        console.log("paths to file " + kmlpoints.length);
        //addPathToFile(false, kmlpoints, index, response, x, y, 0);
