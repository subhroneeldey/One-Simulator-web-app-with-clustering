...
function calcDistanceMatrix() {
    avg_speed = prompt("Average Speed (Km/hr)?", 20);
    if (avg_speed == null)
        return false;
...
var server_xmlhttp = null;

function startServer(sw, ne, rows, cols, stopfirst) {
    if (stopfirst)
        stopServer(true, sw, ne, rows, cols);
    else {
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                var MinLat = parseFloat(array.minlat),
                    MaxLat = parseFloat(array.maxlat),
                    MinLon = parseFloat(array.minlng),
                    MaxLon = parseFloat(array.maxlng);

                edges = [];
                for (var i = 0; i < array.edges.length; i++) {
                    var edgejson = array.edges[i],
                        from = new google.maps.LatLng(edgejson.from.lat, edgejson.from.lng),
                        to = new google.maps.LatLng(edgejson.to.lat, edgejson.to.lng);
                    edges.push(new edge(from, to));
                }
                console.log(edges);

                var latlng = new google.maps.LatLng(MinLat, MinLon);
                calcMaxBoundaries(latlng);
                latlng = new google.maps.LatLng(MaxLat, MaxLon);
                calcMaxBoundaries(latlng);
        ...
        server_xmlhttp = xmlhttp;
    }
}

function stopServer(start, sw, ne, rows, cols) {
    var xmlhttp = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP");
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            console.log(xmlhttp.responseText);
            ...
            if (start) {
                startServer(sw, ne, rows, cols, false);
            }
        }
    }
    xmlhttp.open("POST", "server_close.php", true);
    xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xmlhttp.send();
}